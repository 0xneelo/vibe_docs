# Section 5: The OTC 2 OB Hybrid Architecture

## 5.1 Design Philosophy

The **OTC 2 OB** or **`otc2ob` hybrid** is designed from first principles to solve the Bootstrap Trilemma through temporal separation of concerns. Rather than committing to a single point in the architecture space, it enables markets to **traverse** from bootstrap-capable OTC execution to maximally efficient order-book execution as they mature.
...
The `otc2ob` hybrid operates as a **spectrum system** rather than a fixed architecture:
...
Central to a Permissionless OTC Derivatives Protocol's architecture is the **Solver**—an off-chain operator that provides the critical functions impossible to implement purely on-chain.
...
**Trigger**: Token is created (e.g., on PumpFun), a Permissionless OTC Derivatives Protocol market automatically created
...
T=1: a Permissionless OTC Derivatives Protocol market automatically created
...
**Process**:
1. a Permissionless OTC Derivatives Protocol market reaches graduation criteria
2. Order book is initialized (can be on a Permissionless OTC Derivatives Protocol or external)
3. Solver begins placing quotes in book
4. External market makers can join
5. Liquidity migrates gradually
6. a Permissionless OTC Derivatives Protocol market becomes backstop/bootstrap layer

### 5.6.2 Synergy with External Protocols

a Permissionless OTC Derivatives Protocol creates symbiotic relationships with order book protocols:

**For Hyperliquid/Similar**:
- a Permissionless OTC Derivatives Protocol can provide "pre-qualified" markets
- Data on trading activity, Z-Score, volume
- Reduces listing risk
- Identifies genuine demand

**For a Permissionless OTC Derivatives Protocol**:
- Order book provides graduation path
- Mature markets can operate more efficiently
- Focus resources on bootstrapping

### 5.6.3 Permissionless OTC Derivatives + Order Book = Complete Lifecycle

```
TOKEN LAUNCH → PERMISSIONLESS OTC DERIVATIVE ISSUANCE & BOOTSTRAP → PERMISSIONLESS OTC DERIVATIVE MATURATION → ORDER BOOK
    |               |                |                |
...
**OTC derivative systems, through the operation of propAMMs or propSolvers, are protocols designed for ever-evolving markets, not static operation.**
